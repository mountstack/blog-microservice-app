...
async function handleEvent(type: string, data: any) {
  ...
  if (type === 'PostCreated') {
    try {
      ...
      // web-socket. send data to frontend to all logged-in user
      io && io.emit("post-created", { 
        userId, 
        id: createdPost.id, 
        title: createdPost.title, 
        imageUrl: createdPost.imageUrl, 
        bgColor: createdPost.bgColor, 
        totalComments: createdPost.totalComments, 
      ...
      console.log('[PostCreated]', createdPost); 
    } 
    catch (error: any) {
